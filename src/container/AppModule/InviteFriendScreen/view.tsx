...
import Label from '../../../components/Label';
import MainContainer from '../../../components/MainContainer';
import { inviteFriendNav, navigationProps } from '../../../types/nav';
import Ionicons from 'react-native-vector-icons/Ionicons'
import { OrangeColor } from '../../../assets/colors';
import { firebase } from '@react-native-firebase/dynamic-links';
import Share from 'react-native-share'
...
const InviteFriendScreen: React.FC<inviteFriendNav> = ({
    route
}) => {
    const { leagueDetails } = useSelector((state: RootState) => state.selectedLeague)
    ...
    const createLink = async () => {
        // let code = leagueDetails.unique_code
        const link = await firebase.dynamicLinks().buildShortLink({
            link: `https://fantasysniper?league_id=${route.params?.league_id}&week_id=${route.params?.week_id}`,
            android: {
                packageName: 'com.fantastysport',
            },
            domainUriPrefix: 'https://fantasysport.page.link',
